...
export abstract class StateBufferForSlave<T> extends StateBuffer {
    ...
    protected abstract onNew(index: number): T;

    protected abstract onDelete(index: number, obj: T): void;

    protected abstract onUpdate(index: number, obj: T): void;

    public sync(): boolean {

        const handleObject = (changeIndex: number, objIndex: number, obj: T | undefined): void => {
